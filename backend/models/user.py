...
API calls to update data about the user in firebase. Contains error checking for
errors in calls to Firebase.
...
# Look above one line, changes path to import necessary firebase/pyrebase variables
sys.path.append("../")
from config import db, auth


class User:
    """This class contains all of methods related to users in the firebase database.
    """

    def raise_detailed_error(self, request_object):
        """Used to raise an HTTPError if firebase calls are internally and not through pyrebase.

        Arguments:
            request_object {[type]} -- Recieved from requests.post/get/...() methods.
    ...
    def create_error_message(self, e):
        """Handles exceptions in firebase calls, and returns correct error repsonses.

        Arguments:
            e {Exception} -- Wants an HTTPError exception, but can handle all excpetions.
    ...
    def create_user(
        ...
        """Creates a user with an email and password. Also adds provided info into database.
        ...
        Returns:
            Response object -- If valid call, returns the UID of the user and a status code.
        """
        try:
            ...
            # add data into the users's table with the user's localId as its key
    ...
    def get_user(UID: str):
        ...
        Returns:
            Response object -- If valid call, returns the user's data in the DB and a status code.
        """
        try:
